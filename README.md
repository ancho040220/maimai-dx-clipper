...
이 프로그램은 **YouTube 계정으로 로그인한 본인의 채널 영상만** 분석합니다.
URL을 확인하면 영상의 채널과 로그인한 계정을 대조해, 다른 채널이면 시작이 막힙니다.

| 표시 | 의미 |
|------|------|
| `내 채널` | 본인 영상 — 진행 가능 |
| `다른 채널` | 다른 사람 영상 — 차단 |
| `채널 확인 불가` | 계정 인증 필요 — 환경 점검의 **🔑 재인증** 클릭 |

> 남의 방송을 무단으로 내려받아 재업로드하는 데 쓰이지 않도록 하기 위한 제한입니다.  
> 그래서 아래 `client_secret.json` 설정이 **필수**입니다.

---

## client_secret.json 만드는 법

본인 영상 확인과 자동 업로드에 쓰이는 Google 인증 파일입니다.  
한 번만 만들면 됩니다.
...
4. 요청된 권한(동영상 업로드 · 계정 정보 읽기)에 체크 → **"계속"** 클릭
5. 브라우저 닫으면 자동으로 진행됨
...
| 파일 이름 | 설명 | 갱신 |
|-----------|------|------|
| `client_secret.json` | Google API 인증 (본인 영상 확인 + 업로드) | 한 번만 |
...
> ffmpeg · Tesseract OCR · YOLO 모델 · YouTube 로그인 · Google 인증 파일 · 자켓 인덱스 · yt-dlp · maimai DB · GPU 9개 항목을 확인합니다.
...
| Google 인증 파일 | 필수 | 본인 영상 확인에 필요 (오류 시 시작 불가) |
| 자켓 인덱스 | 조건부 | **곡 정보 추출 ON** 일 때만 필수 (첫 실행 시 자켓 다운로드에 인터넷 필요) |
| yt-dlp | 필수 | 오래되면 YouTube 접근이 막힘 — **⬆️ 업데이트** 버튼으로 갱신 |
...
| 환경 점검 — Google 인증 파일 경고 | `config/credentials/client_secret.json` 준비 여부 확인 |
| 환경 점검 — yt-dlp 새 버전 알림 | **⬆️ 업데이트** 버튼 클릭 후 프로그램 재시작 |
